...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        --increase-volume)
            bash $HOME/.config/eww/widgets/audio/scripts/volume-ctrl.sh --increase-volume
        ;;
        --decrease-volume)
            bash $HOME/.config/eww/widgets/audio/scripts/volume-ctrl.sh --decrease-volume
        ;;
        --mute-toggle)
        ...
        --update-eww)
            ...
            VOLUME=$(bash $HOME/.config/eww/widgets/audio/scripts/volume-ctrl.sh --get-volume)
            BOOSTER=$(bash $HOME/.config/eww/widgets/audio/scripts/volume-ctrl.sh --get-boost-percentage)

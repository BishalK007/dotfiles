...
const handleScanToggle = () => {
    print("Toggling Bluetooth scan...\n");
    const adapter = bt.get_adapter();
    if (adapter) {
        print(`Current discovering state: ${adapter.discovering}\n`);
    ...
    return (
        ...
        >
            ...
            >
                <label
                    label="Adapter:"
                    cssClasses={["bluetooth-adapter-label"]}
                />
                <label
                    label={mergeBindings(
                        [bind(bt, "adapter")],
            ...
            > 
                <button
                    cssClasses={chainedBinding(bt, ["adapter", "discovering"]).as((discovering: boolean) => 
                        discovering 
                    ...
                    child={
                        ...
                        >
                            <label
                                label={chainedBinding(bt, ["adapter", "discovering"]).as((discovering: boolean) => 
                                    discovering ? "󰓦" : "󰍉"
                                )}
                                cssClasses={chainedBinding(bt, ["adapter", "discovering"]).as((discovering: boolean) => 
                                    discovering 
                ...
                <label
                    ...
                    cssClasses={chainedBinding(bt, ["adapter", "discovering"]).as((discovering: boolean) => 
                        discovering ? ["bluetooth-scan-status", "scanning"] : ["bluetooth-scan-status", "not-scanning"]
                    )}
                    hexpand={true}
            ...
            >
                ...
                <box
                    visible={mergeBindings(
                        [bind(bt, "devices")],
                        (devices: AstalBluetooth.Device[]) => {
                            return devices.filter(d => !d.paired && d.name && d.name !== "").length === 0;
                        }
                    )}
                    child={
                        <label
        ...
        if (isPaired) {
            ...
            if (device.connected) {
                device.disconnect_device((source: any, result: any) => {
                    try {
                        ...
                        console.error(`Failed to disconnect from ${device.name}:`, e);
                    }
                });
            } else {
                device.connect_device((source: any, result: any) => {
                    try {
                        ...
                        console.error(`Failed to connect to ${device.name}:`, e);
                    }
                });
            }
        } else {
            ...
            device.pair((source: any, result: any) => {
                try {
                    ...
                    console.error(`Failed to pair with ${device.name}:`, e);
                }
            });
        }
    };
        ...
        if (adapter) {
            adapter.remove_device(device, (source: any, result: any) => {
                try {
                    ...
                    console.error(`Failed to remove ${device.name}:`, e);
                }
            });
        }
    };
                        ...
                        child={
                            <label
                                label={mergeBindings(
                                    ...
                                    (connected: boolean, connecting: boolean, battery: number) => {
                                        if (connecting) return "Connecting...";
                                        if (connected) {
                                            const batteryStr = battery > 0 ? ` (${Math.round(battery * 100)}%)` : "";
                        ...
                        <button
                            cssClasses={mergeBindings(
                                ...
                                (connected: boolean, connecting: boolean, paired: boolean) => {
                                    ...
                                    if (paired) {
                                        if (connecting) return [...baseClasses, "bluetooth-connecting"];
                                ...
                                <label
                                    ...
                                    cssClasses={mergeBindings(
                                        ...
                                        (connected: boolean, connecting: boolean, paired: boolean) => {
                                            ...
                                            if (paired) {
                                                if (connecting) return [...baseClasses, "bluetooth-device-action-icon-connecting"];
                                                return connected 
                                                    ? [...baseClasses, "bluetooth-device-action-icon-disconnect"]
                                                    : [...baseClasses, "bluetooth-device-action-icon-connect"];
                                            }
                                            return [...baseClasses, "bluetooth-device-action-icon-pair"];
                                        }
                                    )}
                                />
                            }
                        ...
                        ...(paired ? [
                                ...
                                cssClasses={mergeBindings(
                                    ...
                                    (trusted: boolean) => 
                                        trusted 
                                            ...
                                            : ["bluetooth-device-trust-button", "bluetooth-not-trusted"]
                                )}
                                ...
                                child={
                                    <label
                                        ...
                                        cssClasses={mergeBindings(
                                            ...
                                            (trusted: boolean) => 
                                                trusted 
                                                    ? ["bluetooth-device-action-icon", "bluetooth-device-action-icon-trusted"]
                                                    : ["bluetooth-device-action-icon", "bluetooth-device-action-icon-untrusted"]
                                        )}
                                    />
                                }
                            />,
                            <button
                                ...
                                child={
                                    <label
                                        label="󰆴"
                                        cssClasses={["bluetooth-device-action-icon", "bluetooth-device-action-icon-remove"]}
                                    />
                                }
                            />
                        ] : [])

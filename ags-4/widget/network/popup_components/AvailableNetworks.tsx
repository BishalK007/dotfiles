...
const logger = createLogger("WidgetNetwork", "AvailableNetworks");
      ...
      <box
        ...
        children={mergeBindings(
          ...
          (
            ...
            activeAP: AstalNetwork.AccessPoint,
          ) => {
            if (!accessPoints) {
              logger.debug("No access points available");
              return [];
            }

            // Sort access points by signal strength (strongest first)
            const sortedAPs = [...accessPoints].sort(
            ...
            const filteredAPs = sortedAPs.filter((ap) => {
              ...
              if (activeAP && ap.ssid === activeAP.ssid) {
                logger.debug(
                  `Filtering out current connection via activeAP: ${ap.ssid}`,
                );
                return false;
              }
              ...
              if (seenSSIDs.has(ap.ssid)) {
                logger.debug(`Filtering out duplicate: ${ap.ssid}`);
                return false;
              }
            ...
            return filteredAPs.map((ap) => (
              <AccessPointItem accessPoint={ap} />
            ));
          },
        )}
      />
  ...
  const handleConnect = () => {
    ...
    accessPoint.activate(null, (source: any, result: any) => {
      try {
        ...
        if (success) {
          logger.success(`Connected to ${accessPoint.ssid}`);
        } else {
          logger.error(`Connection to ${accessPoint.ssid} failed`);
        }
      } catch (e) {
        logger.error(`Failed to connect to ${accessPoint.ssid}: ${e}`);
    ...
    >
      ...
      <label
        ...
        cssClasses={[
          ...
          accessPoint.strength >= 70
            ? "strong"
            : accessPoint.strength >= 50
              ? "good"
              : accessPoint.strength >= 30
                ? "fair"
                : "weak",
        ]}
      ...
      <button
        ...
        child={<label label="󰐕" cssClasses={["network-ap-connect-icon"]} />}
      />
    </box>
  );
}

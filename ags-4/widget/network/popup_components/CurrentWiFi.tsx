...
const logger = createLogger("WidgetNetwork", "CurrentWiFi");
...
export default function CurrentWiFi({ network }: CurrentWiFiProps) {
  ...
  const handleDisconnect = () => {
    if (network.wifi) {
      network.wifi.deactivate_connection((source: any, result: any) => {
        try {
          network.wifi?.deactivate_connection_finish(result);
        } catch (e) {
          logger.error(`Failed to disconnect WiFi: ${e}`);
        }
      }, null);
    }
  };
    ...
    >
      ...
      >
        ...
        >
          {/* Network Name (SSID) */}
          <label
            label={chainedBinding(network, ["wifi", "ssid"]).as(
              (ssid) => ssid || "",
            )}
          ...
          >
            ...
            <label
              label={chainedBinding(network, ["wifi", "strength"]).as(
            ...
            {/* Frequency */}
            <label
              label={chainedBinding(network, ["wifi", "frequency"]).as(
            ...
            {/* Bandwidth */}
            <label
              label={chainedBinding(network, ["wifi", "bandwidth"]).as(
                (bandwidth) => (bandwidth ? formatBandwidth(bandwidth) : ""),
              )}
        ...
        >
          {/* Internet Connectivity Status */}
          <label
            label={chainedBinding(network, ["wifi", "internet"]).as(
              (internet) =>
                internet === AstalNetwork.Internet.CONNECTED ? "󰈁" : "󰈂",
            )}
            cssClasses={chainedBinding(network, ["wifi", "internet"]).as(
              (internet) =>
          ...
          <button
            ...
            child={
              <label label="󰅙" cssClasses={["network-wifi-disconnect-icon"]} />
            }
          />
        </box>
      </box>

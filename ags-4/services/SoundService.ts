...
const SOUNDS = {
    warning: GLib.build_filenamev([PROJ_ROOT, "assets", "sounds", "mixkit-software-interface-back-2575.wav"]),
} as const;
...
export class SoundService {
    ...
    private constructor() {
        ...
        try {
            ...
            print(`Error removing existing socket file: ${e}`);
        }
        ...
        const [success] = this.server.add_address(socketAddress, Gio.SocketType.STREAM, Gio.SocketProtocol.DEFAULT, null);
        ...
        this.server.connect("incoming", (service: Gio.SocketService, connection: Gio.SocketConnection) => {
            this.handleConnection(connection);
            return false; // Return false to let the service continue handling other connections
        });
        
        this.server.start();
        print(`SoundService: Socket server started at ${this.socketPath}`);
    }
    ...
    private async processMessage(message: string, outputStream: Gio.OutputStream) {
        try {
            const parts = message.split(':');
            if (!parts.length) { this.sendResponse(outputStream,'ERROR invalid_format'); return; }
            const soundName = parts[0] as SoundName;
            if (!(soundName in SOUNDS)) { this.sendResponse(outputStream,'ERROR unknown_sound'); return; }
            ...
            if (parts[1] !== undefined && parts[1] !== '') {
                ...
                if (isFinite(d) && d >= 0 && d <= 30) cfg.duration = d; else { this.sendResponse(outputStream,'ERROR bad_value_duration'); return; }
            }
            // emittingSpeaker
            if (parts[2] !== undefined && parts[2] !== '') {
                if (parts[2] === 'all' || parts[2] === 'default') cfg.emittingSpeaker = parts[2] as EmittingSpeaker; else { this.sendResponse(outputStream,'ERROR bad_value_emittingSpeaker'); return; }
            }
            // normaliseVolume
            if (parts[3] !== undefined && parts[3] !== '') {
                const v = parts[3];
                if (['true','1'].includes(v)) cfg.normaliseVolume = true; else if (['false','0'].includes(v)) cfg.normaliseVolume = false; else { this.sendResponse(outputStream,'ERROR bad_value_normaliseVolume'); return; }
            }
            // normaliseVolumeLevel
            if (parts[4] !== undefined && parts[4] !== '') {
                const n = Number(parts[4]);
                if (isFinite(n) && n >= 0 && n <= 1) cfg.normaliseVolumeLevel = n; else { this.sendResponse(outputStream,'ERROR bad_value_normaliseVolumeLevel'); return; }
            }
            // volumeRestoreDelayMs
            if (parts[5] !== undefined && parts[5] !== '') {
                const n = Number(parts[5]);
                if (Number.isInteger(n) && n >= 0 && n <= 10000) cfg.volumeRestoreDelayMs = n; else { this.sendResponse(outputStream,'ERROR bad_value_volumeRestoreDelayMs'); return; }
            }
    ...
    private async run(cmd: string): Promise<string> {
        try { return (await execAsync(['bash','-c',cmd])).trim(); } catch (e) { print(`SoundService run error: ${cmd} -> ${e}`); return ""; }
    }
        ...
        try {
            ...
            for (const o of arr) {
                ...
                if (props?.['media.class'] === 'Audio/Sink' && props['node.name']) {
                    const id = String(o.id ?? props['object.serial'] ?? props['node.id'] ?? props['node.name']);
                    sinks.push({ id, name: props['node.name'] });
                }
            }
            return sinks;
        } catch (e) {
            print(`SoundService: Failed to parse pw-dump JSON: ${e}`);
            return [];
        }
    }
    ...
    private async captureCurrentVolumes(sinks: {id:string,name:string}[]): Promise<Map<string,string>> {
    ...
    private async normaliseVolumes(sinks: {id:string,name:string}[], level: number) {
        for (const s of sinks) {
    ...
    private async playWithConfig(soundName: SoundName, duration: number, cfg: RuntimeSettings) {
        const soundPath = SOUNDS[soundName];
        const file = Gio.File.new_for_path(soundPath);
        if (!file.query_exists(null)) { print(`Sound file not found: ${soundPath}`); return; }

        try {
            const sinks = await this.listSinkNodes();
            if (!sinks.length) print('SoundService: No sinks found, falling back to default pw-play');

            let targetSinks: {id:string,name:string}[] = sinks;
            if (cfg.emittingSpeaker === 'default' && sinks.length) targetSinks = [sinks[0]];
            ...
            const timeoutPrefix = (duration && duration > 0) ? `timeout ${duration}s ` : '';
            if (cfg.emittingSpeaker === 'all' && targetSinks.length > 1) {
                const bashParts = targetSinks.map(s => `${timeoutPrefix}pw-play --target "${s.name}" "${soundPath}"`);
                await execAsync(['bash','-c', bashParts.map(c=>`(${c})`).join(' & ') + ' ; wait || true']);
            } else {
                const single = targetSinks[0];
                const cmd = single ? `${timeoutPrefix}pw-play --target "${single.name}" "${soundPath}"` : `${timeoutPrefix}pw-play "${soundPath}"`;
                await execAsync(['bash','-c',cmd]);
            }

            if (savedVolumes) {
                GLib.timeout_add(GLib.PRIORITY_DEFAULT, cfg.volumeRestoreDelayMs, () => {
    ...
    public static sendSoundRequest(message: string) {
        try {
            ...
            const socketAddress = new Gio.UnixSocketAddress({ path: "/tmp/play-sound.sock" });
    ...
    public cleanup() {
        ...
        try {
            ...
            print(`Error cleaning up socket file: ${e}`);
        }
    }
}

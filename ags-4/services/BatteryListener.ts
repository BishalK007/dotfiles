...
const logger = createLogger("BatteryWarning");
...
class BatteryWarningServiceClass {
  ...
  private handleChargingStateChange(): void {
    ...
    if (!this.isCharging && newChargingState) {
      this.triggeredLevels.clear();
      logger.info("Charging started, reset triggered levels");
    }
  ...
  private handleBatteryChange(): void {
    ...
    logger.debug(
      `Battery change: ${currentPercentage}% (was ${this.lastPercentage}%), charging: ${isCharging}`,
    );
  ...
  private checkWarningLevels(currentPercentage: number): void {
    ...
    for (const level of warningLevels) {
      ...
      if (
        ...
        logger.info(
          `Added level ${level.percentage}% to triggered levels. Current triggered: [${Array.from(this.triggeredLevels).join(", ")}]`,
        );
      }
    }
  }

  private triggerBatteryWarning(
    level: BatteryWarningLevel,
    currentPercentage: number,
  ): void {
    logger.warn(
      `Triggering ${level.isCritical ? "critical" : "normal"} warning at ${currentPercentage}% (threshold: ${level.percentage}%)`,
    );
  ...
  public resetTriggeredLevels(): void {
    this.triggeredLevels.clear();
    logger.info("Manually reset triggered levels");
  }
